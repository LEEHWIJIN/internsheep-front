  ...
  export default{
      ...
      methods: {
          ...
          loadFileName(loginId){
              this.$http.get('API_SERVER/std/mypage/loadFileName',{params:{sLoginID : loginId}}).then((response)=>{
                  if(response.data == '0'){
                      this.fileName = ""
                  }
          ...
          submitFileAndReview(){
            ...
            // } //이렇게 key 값과 value값을 확인하면 확인 가능하다.
              ...
              if(this.review==''){
                  this.$http.post('API_SERVER/std/mypage/postReview', data, config).then(
                      (response) => {
                      }
                  )
              }
