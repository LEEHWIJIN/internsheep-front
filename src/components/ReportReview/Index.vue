  ...
  export default{
      ...
      methods: {
          downloadButton(loginId){
              this.url = 'http://api.ajou-internsheep.co/std/mypage/downloadReport?sLoginID='+loginId
              this.loadFileName(loginId)
          },
          loadFileName(loginId){
              this.$http.get('http://api.ajou-internsheep.co/std/mypage/loadFileName',{params:{sLoginID : loginId}}).then((response)=>{
                  if(response.data == '0'){
                      this.fileName = ""
                  }
          ...
          loadReview(loginId){
              this.$http.get('http://api.ajou-internsheep.co/std/mypage/watchReview',{params:{sLoginID : loginId}}).then((response)=>{
                  if(response.data != '0'){
          ...
          submitFileAndReview(){
            ...
            // } //이렇게 key 값과 value값을 확인하면 확인 가능하다.
              ...
              if(this.fileName==''){
                  this.$http.post('http://api.ajou-internsheep.co/std/mypage/postReport', data, config).then(
                      (response) => {
              ...
              if(this.review==''){
                  this.$http.post('http://api.ajou-internsheep.co/std/mypage/postReview', data, config).then(
                      (response) => {
                      }
                  )
              }
              if(this.uploadFile == null){
                  this.$http.post('http://api.ajou-internsheep.co/std/mypage/modifyReport', data, config).then(
                      (response) => {
