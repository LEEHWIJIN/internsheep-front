  ...
  export default{
      ...
      methods: {
          ...
          applyStatus(loginId){
              ...
              this.$http.get('http://10.41.83.255:8888/std/mypage/applyStatus',{params:{sLoginID : loginId, applySemester : this.applyTerm.applySemester}}).then((response)=>{
          ...
          // }
          loadReview(loginId){
              this.$http.get('http://10.41.83.255:8888/std/mypage/watchReview',{params:{sLoginID : loginId}}).then((response)=>{
                  if(response.data != '0'){
                      this.confirm = 1
          ...
          submitFileAndReview(){
            ...
            // } //이렇게 key 값과 value값을 확인하면 확인 가능하다.
              if(this.confirm == 0) {
                  this.$http.post('http://10.41.83.255:8888/std/mypage/postReview',{sLoginID : this.user.loginId, starScore : startScoreValue, reviewContent : this.review, reviewTitle : this.reviewTitle}).then(
                      response => {
                      }
                  )
              }
              else{
                  this.$http.post('http://10.41.83.255:8888/std/mypage/modifyReview', {sLoginID : this.user.loginId, starScore :  startScoreValue, reviewContent : this.review, reviewTitle : this.reviewTitle}).then(
                      response => {
                      }
                  )
              }
