...
import 'bootstrap-vue/dist/bootstrap-vue.css'
  export default{
      ...
      async created(){
          ...
          await this.$http.get('API_SERVER/std/mypage/checkReportTerm',{params:{sLoginID : this.user.loginId, applySemester: this.applyTerm.applySemester}}).then(res => {
      ...
      methods: {
          ...
          //     this.$http.get('API_SERVER/std/mypage/applyStatus',{params:{sLoginID : loginId, applySemester : this.applyTerm.applySemester}}).then((response)=>{
          ...
          // }
          loadReview(loginId){
              this.$http.get('API_SERVER/std/mypage/watchReview',{params:{sLoginID : loginId}}).then((response)=>{
                  if(response.data != '0'){
                      this.confirm = 1
          ...
          submitFileAndReview(){
            ...
            // } //이렇게 key 값과 value값을 확인하면 확인 가능하다.
              if(this.confirm == 0) {
                  this.$http.post('API_SERVER/std/mypage/postReview',{sLoginID : this.user.loginId, starScore : startScoreValue, reviewContent : this.review, reviewTitle : this.reviewTitle}).then(
                      response => {
                        alert("후기가 작성되었습니다.")
                      }
                  )
                  ...
                  this.$http.post('API_SERVER/std/mypage/modifyReview', {sLoginID : this.user.loginId, starScore :  startScoreValue, reviewContent : this.review, reviewTitle : this.reviewTitle}).then(
                      response => {
                        alert("후기가 수정되었습니다.")
                      }
                  )
