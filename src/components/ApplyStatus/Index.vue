  ...
  export default{
    ...
    methods: {
      ...
      getApplyStatus(loginId) {
        this.$http.get('API_SERVER/std/mypage/loadApplyAll',{params:{sLoginID : loginId, applySemester : this.applyTerm.applySemester}}).then((response)=>{
        ...
        giveup(index){
          ...
          if(this.applylist[index].YN == '심사중'){
            this.$http.post('API_SERVER/std/mypage/giveup',{sLoginID : this.user.loginId, applySemester : this.applyTerm.applySemester, applyOrder : this.applylist[index].applyOrder}).then((response) => {
                alert('포기되었습니다.')
