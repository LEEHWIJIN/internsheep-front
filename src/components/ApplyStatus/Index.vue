      ...
      getApplyStatus(loginId) {
        ...
        giveup(index){
          ...
          if(this.applylist[index].YN == '심사중'){
            this.$http.post('http://106.10.46.121:10022/std/mypage/giveup',{sLoginID : this.user.loginId, applySemester : this.applyTerm.applySemester, applyOrder : this.applylist[index].applyOrder}).then((response) => {
                alert('포기되었습니다.')
