    ...
    methods:{
        ...
        signup(){
            if(this.Isuniq==-1) alert("중복을 확인하지 않았습니다. 확인하세요")
                ...
                this.$http.post('http://api.ajou-internsheep.co/auth/std/signup', {user: this.user}).then((response) => {
        ...
        dupcheck(){
            this.$http.get('http://api.ajou-internsheep.co/auth/std/dupcheck', {params:{id : this.user.id}}).then((res)=>{
